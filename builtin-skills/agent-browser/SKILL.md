...
```bash
# agent-browser is bundled with the Industry CLI -- no install required.
...
**CRITICAL: Always close your browser session with `agent-browser close`
when your task is complete.** Failing to close leaks browser processes and
system resources. Use a single session per task -- do not open multiple
sessions unless explicitly required (e.g., multi-user testing).
...
- Industry optimizes browser usage for the agent-browser CLI, which may conflict with Chrome DevTools MCP.
